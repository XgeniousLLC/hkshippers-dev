...
<div class="mail-container">
    ...
    <div class="content-wrapper" style="background:#fff;border-color:#5645ff;text-align:left;" >
        ...
        @php 
        $template = \App\Post::where(['type'=>'email-templates','type2'=>'contact-us'])->orderBy('seq_no', 'asc')->orderBy('id', 'desc')->first();
        if(!empty($template)){
            $content = $template->content;
            $content = \App\Http\Controllers\ApplicationFormController::replace($content,$item);
            echo $content;
        }
        @endphp
    </div>
