<script>
    ;(function () {
        ...
        $(document).on('click','.media_upload_modal_submit_btn',function (e) {
            ...
            if( typeof allData != 'undefined'){
                ...
                $.each(allData,function(index,value){
                    ...
                    var img = '<img src="'+el.imgsrc+'">';
                    if(el.size===null){
                        img = '<i class="fas fa-file file-icon"></i>';
                    }
                    mainUploadBtn.parent().find('.img-wrap').append('<div class="attachment-preview"><div class="thumbnail"><div class="centered">'+img+'</div></div></div>');
                });
                 mainUploadBtn.prev('input').val(imageId.substring(0,imageId.length -1));

            }
            $('#media_upload_modal').modal('hide');
            $('.media_upload_form_btn').text('Change File');
        });
        ...
        $(document).on('click','.media_upload_form_btn',function (e) {
            ...
            var filetype = el.attr('data-filetype');
            mainUploadBtn = el;
            var loadAllImage = $('#load_all_media_images');
            if(filetype){
                loadAllImage.attr('data-filetype',filetype);
        ...
        function loadAllImages() {
            ...
            var url = "{{route('admin.upload.media.file')}}";
            var filetype = $('#load_all_media_images').attr('data-filetype');
            if(filetype){
                url += '/type/'+filetype;
            }else{
                url += '/all';
        ...
        /* loadmore image  */
            ...
            var url = "{{route('admin.upload.media.file.loadmore')}}";
