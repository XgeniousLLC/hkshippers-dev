                            ...
                            <div class="row">
                                <div class="col-lg-8">
                                    ...
                                    <small> {{__('recommended image size is')}} <span class="dimentions">{{$dimentions[$post->location][0].' x '. $dimentions[$post->location][1]}}</span> </small>
                                </div>
                                <div class="col-lg-4">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="location" id="location" class="form-control" onchange="var obj = {{json_encode($dimentions)}};var val =  $(this).val(); $('.dimentions').text(obj[val][0]+' x '+obj[val][1]);">
                                        
                                        @foreach(\App\Http\Controllers\PostController::LOCATION as $type)
                                                <option @if((int)$post->location === (int)$type[0]) selected @endif value="{{$type[0]}}">{{$type[1]}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
