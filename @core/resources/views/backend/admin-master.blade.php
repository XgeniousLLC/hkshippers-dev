...
<head>
    ...
    <style>
        .user-profile{
            background: #214284;
            background: linear-gradient(to right, #2d54a1 0%, #214284 100%);
        }
        .main-menu{
            
            background: #057dc1;
            background: linear-gradient(to top, #214284 0%, #057dc1 100%);
        }
        .metismenu li a,
            ...
            <div class="row align-items-center">
                ...
                <div class="col-md-6 col-sm-4 clearfix">
                    <ul class="notification-area pull-right">
                        ...
                        <li><a class="btn btn-primary"  href="{{ route('admin.logout') }}" 
...
<script>
    ...
    (function($){
        ...
        $(document).ready(function ($) {
            $.each($('.summernote'),function(i,obj){
                var input = $(obj).prev('input,textarea');
                var name = input.attr('name');
                var val = input.val();
                val = val.replace(/[<]((?:(?![>]).))+[>]/g,'',val);

                $('<textarea class="form-control" style="height:500px;" name="'+name+'">'+val+'</textarea>').insertBefore(input);
                input.remove();
            })
            ...
            $('._summernote').summernote({
                ...
                callbacks: {
                    onChange: function(contents, $editable) {
                        ...
                        $(this).prev('[name]').val(finalContenat);
                    }
                }
            });
            if($('._summernote').length >= 1){

                $('._summernote').each(function(index,value){
