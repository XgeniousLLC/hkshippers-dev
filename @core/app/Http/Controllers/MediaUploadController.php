...
class MediaUploadController extends Controller
{
    ...
    public function index(){
        $all = MediaUpload::where([
        ])->orderBy('id', 'desc')->orderBy('created_at', 'desc')->get();

        
        return view('backend.pages.media-room.index')->with([
            'title' => 'Media Room',
            'all' => $all
        ]);
    }
    ...
    public function get_upload_media_file(Request $request,$filetype)
    {
        $db =  MediaUpload::orderBy('id', 'DESC')->take(20);
        $arr = explode('|',$filetype);
        foreach($arr as $type){
            $db->orWhere('title','like','%'.$type);
        }
        $all_images = $db->get();
        $selected_image = MediaUpload::find($request->selected);
        $all_image_files = [];
    ...
    {
        $get_image_details = MediaUpload::find($id);
        if (file_exists('assets/uploads/media-uploader/'.$get_image_details->path)){
            unlink('assets/uploads/media-uploader/'.$get_image_details->path);
        }
        if (file_exists('assets/uploads/media-uploader/grid-'.$get_image_details->path)){
        ...
        return MediaUpload::find($id)->delete();
    ...
    public function get_media_for_loadmore(Request $request){
    ...
    public function get_media_for_loadmore_filetype(Request $request,$filetype){
        $db = MediaUpload::orderBy('id', 'DESC')->skip($request->skip);
        $arr = explode('|',$filetype);
        
        foreach($arr as $type){
            $db->orWhere('title','like','%'.$type);
        }
        $all_images = $db->take(20)->get();
        $all_image_files = [];
        foreach ($all_images as $image){
            if (file_exists('assets/uploads/media-uploader/'.$image->path) && !is_dir('assets/uploads/media-uploader/'.$image->path)){
                $image_url = asset('assets/uploads/media-uploader/'.$image->path);
