...
class FrontendController extends Controller
{
    ...
    public function index()
    {
        ...
        $all_blog = Post::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->take(6)->get();
        ...
        if ($home_page_variant == '20'){
           ...
           //advertisement code top section
            ...
            $blade_data['add_id'] = $add??$add->id;

            $image_markup = render_image_markup_by_attachment_id($add??$add->image,null,'full');
            $redirect_url = $add??$add->redirect_url;
            $slot = $add??$add->slot;
            $embed_code = $add??$add->embed_code;
    ...
    public function product_story_item_by_category(Request $request){
      ...
      if(!empty($item->regular_price) && $item->regular_price > 0){
        $regular_price_markup = '<span class="fs-16 flash-old-prices ff-rubik"> '.$regular_price.' </span>';
      }
