...
class EventsController extends Controller
{
    ...
    protected const VALIDATOR = [
        ...
        'seq_no' => 'integer',
        'limit_online' => 'nullable|integer',
        'limit_in_person' => 'nullable|integer',
        'form_type' => 'nullable',
        'language' => 'nullable',
        'speaker' => 'nullable',
    ...
    public function new_event($type){
        ...
        $form = 'backend.events.partial.new-form1';
        ...
        return view('backend.events.new-event')->with([
    ...
    public function store_event(Request $request,$type){
        $all_languages = Language::all();
        
        $default_lang = LanguageHelper::default()->slug;
        $validator = SELF::VALIDATOR;
        foreach(SELF::VALIDATOR as $name => $val){
            $_name = $name.'-'.$default_lang;
            if(isset($request->$_name)){
                unset($validator[$name]);
                 $validator[$_name] = $val;
                $request->$name = $request->$_name;
                foreach($all_languages as $lang){
                    $lang = $lang->slug;
                    if($lang!==$default_lang){
                        $_name = $name.'-'.$lang;
                        $validator[$_name] = 'nullable';
                    }
                }
            }
        ...
        $event = Events::create([
            ...
            'limit_online' => $request->limit_online,
            'limit_in_person' => $request->limit_in_person,
    ...
    public function all_events($type){
        ...
        $title = '';
        $list = 'backend.events.partial.list1';
        ...
        return view('backend.events.all-events')->with([
            'title' => $title,
            'type' => $type,
            'list' => $list,
    ...
    public function edit_event($id){
        ...
        $all_languages = Language::all();
        $form = 'backend.events.partial.edit-form1';
    ...
    public function clone_event(Request $request){
        ...
        $item = Events::create([
            ...
            'status' => 'draft',
            'fee' => $event_details->fee,
            'type' => $event_details->type,
            'description' => $event_details->description,
            'speaker' => $event_details->speaker,
            'language' => $event_details->language,
            'form_type' => $event_details->form_type,
            'limit_in_person' => $event_details->limit_in_person,
            'limit_online' => $event_details->limit_online,
            'seq_no' => $event_details->seq_no,
            'attachment' => $event_details->attachment,
        ...
        $meta = Meta::where('event_id',$request->item_id)->get();
		foreach($meta as $m){
			Meta::insert([
				'value'=>$m->value,
				'name'=>$m->name,
        ...
        return redirect()->back()->with(['msg' => __('Items Clone Success...'),'type' => 'success']);
    }
    ...
    static public function replace($str,Events $event,$app = null){
        preg_match_all('/\[[a-zA-Z\s0-9\.]+\]/',$str,$matches);
        $arr = [
            'description'=>'content',
            
            'participants'=>'guest',
            'no. of participant'=>'qty',
        ];
        ...
        foreach($matches[0] as $m){
            ...
            if(@$event->$_m){
                $str = str_replace($m,$event->$_m,$str);
            }elseif($app&&@$app[$_m]){
                $str = str_replace($m,$app[$_m],$str);
            }else{
